...
describe("Create/Update Comments", () => {
  ...
  it("Should find the user Donald Duck", async () => {
    ...
    const result = await fetch(`${URL}/api/users/${userName}`).then(r => r.json());
  ...
  it("Should Remove the user Donald Duck", async () => {
    const userName = 'dd@b.dk';
    const listBeforeRemoving = await fetch(`${URL}/api/users`).then(r => r.json());
    const config = {
      method: 'DELETE',
      headers: {
        'Accept': 'application/json',
    ...
    await fetch(`${URL}/api/users/${userName}`, config).then(r => r.json());
    const listAfterRemoving = await fetch(`${URL}/api/users`).then(r => r.json());
    expect(listAfterRemoving.length).to.be.equal(2);
    expect(listAfterRemoving.length).to.be.equal(listBeforeRemoving.length-1);
   })
})
